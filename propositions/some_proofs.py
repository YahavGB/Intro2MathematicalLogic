...
def prove_and_commutativity() -> Proof:
    ...
    """
    builder = ProofBuilder.claim('(p&q)', '(q&p)') \
        .with_rule(A_RULE) \
        .with_rule(AE1_RULE) \
        .with_rule(AE2_RULE)
        
    return builder.add_assumption_line('(p&q)') \
            .add_claim_line('q', AE1_RULE, [0]) \
            .add_claim_line('p', AE2_RULE, [0]) \
            .add_claim_line('(q&p)', A_RULE, [1, 2]) \
            .build()
...
def prove_I0() -> Proof:
    ...
    builder = ProofBuilder(I0) \
        .with_rule(MP) \
        .with_rule(I1) \
        .with_rule(D)
    
    return builder \
        .add_claim_line('(p->((p->p)->p))', I1) \
        .add_claim_line('((p->((p->p)->p))->((p->(p->p))->(p->p)))', D) \
        .add_claim_line('((p->(p->p))->(p->p))', MP, [0, 1]) \
        .add_claim_line('(p->(p->p))', I1) \
        .add_claim_line('(p->p)', MP, [3, 2]) \
        .build()
...
def prove_hypothetical_syllogism() -> Proof:
    ...
    """
    # Task 5.5
    statement_to_proof = InferenceRule([Formula.parse('(p->q)'), Formula.parse('(q->r)'), Formula.parse('p')],
                                       Formula.parse('r'))

    p = ProofBuilder(statement_to_proof) \
        .with_rules({MP, I0, I1, D}) \
        .add_assumption_line('p') \
        .add_assumption_line('(p->q)') \
        .add_claim_line('q', MP, (0, 1)) \
        .add_assumption_line('(q->r)') \
        .add_claim_line('r', MP, (2, 3)) \
        .build()
