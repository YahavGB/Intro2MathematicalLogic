...
def test_all_models(debug=False):
    ...
    for variables,models in [[variables1, models1], [variables2, models2]]:
        if debug:
            ...
            print('\tGot:      ', list(all_models(variables)))
            print('\tExpected: ', models)
            if list(all_models(variables)) == models:
                print('\tSUCCESS\n')
            else:
                print('\tFAILED!!\n')
...
def test_print_truth_table(debug=False):
    ...
    # print_truth_table(Formula.parse(infix1))
...
def test_synthesize(debug=False):
    ...
    for all_variables, all_models,value_lists in [
        ...
        for all_values in value_lists:
            ...
            if not is_DNF(formula):
                print('\n\nFromula is not a DNF:')
...
def test_evaluate_inference(debug=False):
    ...
    for model in all_models(['p', 'q', 'r']):
        ...
        if not evaluation_result:
            print("FAILED.")
            print("*" * 25)
            print("EXPECTED:", (not model['p']) or (not model['q']) or model['r'])
            print("REASON:  ", "(not {}) or (not {}) or {}".format(model['p'], model['q'], model['r']))
            print("*" * 25)
...
def test_is_sound_inference(debug=False):
    ...
    for assumptions,conclusion,tautological in [
        ...
        val = is_sound_inference(rule)
        if val != tautological:
            print("FAILED.")
            print('*'*25)
            print("GOT:         ", val)
            print("EXPECTED:    ", tautological)
            print('*'*25)
