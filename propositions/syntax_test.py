...
def test_repr(debug=False):
    ...
    # print(str(Formula("->",
    #         Formula("~", Formula("|", Formula("p"), Formula("~", Formula("q")))),
    #         Formula("&", Formula("&", Formula("x"), Formula("y")), Formula("|", Formula("u"), Formula("v"))
    #         ))))
...
def test_operators(debug=False):
    for f, ops in [(Formula('T'), {'T'}),
        ...
        if debug:
            print("Testing ops of", f)
            print("\tExpected: {}\n\tGot: {}".format(ops, f.operators()))
            if f.operators() == ops:
                print("\tSUCCESS")
            else:
                print("\tFAILED")
...
def test_parse_prefix(debug=False):
    if(debug):
        print()
        # print(Formula.parse_prefix("(x13->x14)"))
        # sys.exit(0)
...
def test_substitute_operators(debug=False):
    ...
    for f, d, r in tests:
        ...
        assert a == r, "Incorrect answer:"+a + " expected " + r
