...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if (formula_type is Formula.FormulaType.T_RELATION and formula.root not in relations_instantiation_map) \
        ...
        if formula_type is Formula.FormulaType.T_RELATION:
            if len(formula.arguments) != 1:
                instantiation_value = relations_instantiation_map[formula.root]
                if hasattr(instantiation_value, 'arguments'):
                    # Make sure that we didn't got any forbidden variable in here
                    try:
                        for arg in instantiation_value.arguments:
                            Schema.__assert_not_forbidden(arg, bound_variables)
                    except ForbiddenVariableError as e:
                        raise Schema.BoundVariableError(e.variable_name, formula.root)

                return instantiation_value
            elif len(formula.arguments) == 1:
                # Perform the substitution on this value
                substituted_value = formula.arguments[0].substitute(constants_and_variables_instantiation_map, set())
                instantiation_value = relations_instantiation_map[formula.root]

                # Assert that there's no bad value
                if hasattr(instantiation_value, 'arguments'):
                    try:
                        for arg in instantiation_value.arguments:
                            Schema.__assert_not_forbidden(arg, bound_variables)
                ...
                # Perform the actual main substitution
                try:
                    return instantiation_value.substitute({TermToken.T_UNDERSCORE.value: substituted_value}, set())
                except ForbiddenVariableError as e:
                    raise Schema.BoundVariableError(e.variable_name, formula.root)
        ...
        if formula_type is Formula.FormulaType.T_QUALIFIER:
            # Our variable is one of the actual instantiated variables/consts?
            if formula.variable in constants_and_variables_instantiation_map:
                formula = Formula(formula.root, str(constants_and_variables_instantiation_map[formula.variable]),
                                  formula.predicate)
            ...
            return Formula(formula.root, formula.variable,
                           Schema._instantiate_helper(formula.predicate,
                                                      constants_and_variables_instantiation_map,
                                                      relations_instantiation_map,
                                                      bound_variables | {formula.variable}))
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        # Build the const, variables and relations map
        const_and_variables_map = {}
        relations_map = {}

        for k, val in instantiation_map.items():
            if k not in self.templates:
                return None

            if is_constant(k):
                const_and_variables_map[k] = val
            elif is_variable(k):
                const_and_variables_map[k] = Term.parse(val)
            elif is_relation(k):
                relations_map[k] = val

        try:
            return Schema._instantiate_helper(self.formula, const_and_variables_map, relations_map)
    ...
    @staticmethod
    def __assert_not_forbidden(t: Term, forbidden_variables: Set[str]):
        """
        Asserts that the given term does not contain any forbidden variable.

        Parameters:
                term The term to check.
                forbidden_variables The set of foribdden variables to assert against.
        """
        if is_constant(t.root) or is_variable(t.root):
            if t.root in forbidden_variables:
                raise ForbiddenVariableError(t.root)
        if is_function(t.root):
            for arg in t.arguments:
                Schema.__assert_not_forbidden(arg, forbidden_variables)
...
class Proof:
    ...
    class ProofBuilder:
        """
        Provides a builder pattern based utility class used to build a proof.
        """
        assumptions: Set[Schema]
        conclusion: Formula
        lines: List[Proof.Line, ...]

        def __init__(self, conclusion: Optional[Formula] = None, assumptions: Set[Schema] = None) -> None:
            """Initializes a Proof builder.

            Parameters:
                conclusion: The conclusion for the proof.
                assumptions: The assumptions scheme."""
            self.conclusion = conclusion
            self.assumptions = assumptions
            self.lines = list()

        def with_conclusion(self, conclusion: Formula) -> Proof.ProofBuilder:
        ...
        @staticmethod
        def claim(assumptions: Set[Schema], conclusion: Union[str, Formula]) -> Proof.ProofBuilder:
            """Builds a new claim from the given assumption, which aims to proof the given conclusion.

            Parameters:
                assumptions: An assumption scheme.
            ...
            return Proof.ProofBuilder(conclusion, assumptions)

        def with_assumptions(self, assumptions: Set[Schema]) -> Proof.ProofBuilder:
            """
            Set the assumptions this proof builder is based on.
        ...
        def add_line(self, line: Proof.Line) -> Proof.ProofBuilder:
        ...
        def add_claim_line(self, formula: Formula, assumption: Schema, instantiation_map: InstantiationMap):
            """
            Adds a line justified by an assumption.
            ...
            Returns:
                The builder object.
            """
            self.lines.append(Proof.AssumptionLine(formula, assumption, instantiation_map))
            return self

        def add_mp_line(self, formula: Formula, antecedent_line_number: int, conditional_line_number: int):
            """
            Adds a line justified by the Modus Ponens (MP) inference.
            ...
            Returns:
                The builder object.
            """
            self.lines.append(Proof.MPLine(formula, antecedent_line_number, conditional_line_number))
            return self

        def add_lines(self, lines: Tuple[Proof.Line]) -> Proof.ProofBuilder:
        ...
        def build(self) -> Proof:
            """
            Builds the proof object.

            Returns:
                The built proof object.
            """
            return Proof(self.assumptions, self.conclusion, self.lines)
    ...
    class AssumptionLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            for assumption in assumptions:
                if assumption.instantiate(self.instantiation_map) == lines[line_number].formula:
                    return True
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if line_number < self.antecedent_line_number or line_number < self.conditional_line_number:
                return False

            p = lines[self.antecedent_line_number].formula
            q = lines[line_number].formula
            implies_statement = lines[self.conditional_line_number].formula

            return p == implies_statement.first and q == implies_statement.second
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            return lines[line_number].formula.get_type() is Formula.FormulaType.T_QUALIFIER \
                and lines[line_number].formula.root == FormulaToken.T_QUALIFIER_FORALL.value \
                and lines[line_number].formula.predicate == lines[self.predicate_line_number].formula
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            formula, _ = lines[line_number].formula.propositional_skeleton()
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    return {k.capitalize(): Formula.from_propositional_skeleton(v, substitution_map)
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    skeleton = tautology.propositional_skeleton()
    proof = prove_propositional_tautology(skeleton[0], {})
    return prove_from_skeleton_proof(tautology, proof, skeleton[1])
