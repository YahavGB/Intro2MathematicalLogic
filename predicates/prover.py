...
class Prover:
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str],
        ...
        return self.add_mp(instantiation, line_number,
                           self.add_instantiated_assumption(
                               Formula.FormulaBuilder.implies(quantified, instantiation),
                               Prover.UI,
                               {  # Seed the UI axiom:
                                   'x': quantified.variable,
                                   'R': str(quantified.predicate.substitute({quantified.variable: Term.parse('_')})),
                                   'c': str(term)
                               }
                           ))
    ...
    def add_tautological_implication(self, implication: Union[Formula, str],
        ...
        parts = [(implication, 0)] + [(self._lines[l].formula, l) for l in line_numbers]

        # Create the implications chain
        implications_chain_formula = parts[0][0]
        for i in range(1, len(parts)):
            assert parts[i][0] is not None
            implications_chain_formula = Formula.FormulaBuilder.implies(parts[i][0], implications_chain_formula)

        # Create the last step, which describes the implications chain
        step = self.add_tautology(implications_chain_formula)

        # Reverse iterate and insert every MP claim
        for i in range(len(parts) - 1, 0, -1):
            implications_chain_formula = implications_chain_formula.second
            step = self.add_mp(implications_chain_formula, parts[i][1], step)
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        # Setup
        existence_assumption = self._lines[line_number1].formula
        consequence_result = self._lines[line_number2].formula

        # Create the UG Claim by forming the variable and the consequence result
        step1 = self.add_ug(Formula.FormulaBuilder.forall(existence_assumption.variable,
                                                          consequence_result), line_number2)

        # Assume the main implication
        step2 = self.add_instantiated_assumption(
            Formula.FormulaBuilder.implies(
                Formula.FormulaBuilder.binary_operator(
                    FormulaToken.T_AND.value,
                    Formula.FormulaBuilder.forall(existence_assumption.variable, consequence_result),
                    existence_assumption
                ),
                consequence_result.second
            ),
            Prover.ES, {
                'R': str(existence_assumption.predicate).replace(existence_assumption.variable, '_'),
                'Q': consequence_result.second,
                'x': existence_assumption.variable
            })

        # Return the last step - the tautological implication
        return self.add_tautological_implication(consequence_result.second, {line_number1, step1, step2})
    ...
    def add_flipped_equality(self, flipped: Union[Formula, str],
        ...
        lhs = str(flipped.arguments[1])
        rhs = str(flipped.arguments[0])

        # Create the main implication statement
        implication_argument = Formula.FormulaBuilder.implies(
            Formula.parse('%s%s%s' % (lhs, FormulaToken.T_EQUAL.value, rhs)),
            Formula.FormulaBuilder.implies(
                Formula.parse('%s%s%s' % (lhs, FormulaToken.T_EQUAL.value, lhs)),
                Formula.parse('%s%s%s' % (rhs, FormulaToken.T_EQUAL.value, lhs))
            )
        )

        # Compute the flipping steps
        step1 = self.add_instantiated_assumption(implication_argument, Prover.ME, {
            'R': Formula.FormulaBuilder.equality(Term.parse(TermToken.T_UNDERSCORE.value), Term.parse(lhs)),
            'c': lhs,
            'd': rhs
        })
        step2 = self.add_mp(implication_argument.second, line_number, step1)
        step3 = self.add_instantiated_assumption('%s%s%s' % (lhs, FormulaToken.T_EQUAL.value, lhs), Prover.RX,
                                                 {'c': lhs})

        return self.add_mp(implication_argument.second.second, step3, step2)
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        substitution_map = {
            variable: (term, Term.parse(next(fresh_variable_name_generator)))
            for variable, term in substitution_map.items()
        }

        # Apply the line_number
        current_formula = self._lines[line_number].formula
        step = line_number

        for variable in substitution_map:
            step = self.add_ug(Formula.FormulaBuilder.forall(variable, current_formula), step)
            current_formula = current_formula.substitute({variable: substitution_map[variable][1]})
            step = self.add_universal_instantiation(current_formula, step, substitution_map[variable][1])

        # Build the new substitution map
        new_map = {value[1]: value[0] for _, value in substitution_map.items()}
        for variable in new_map:
            step = self.add_ug(Formula.FormulaBuilder.forall(str(variable), current_formula), step)
            current_formula = current_formula.substitute({str(variable): new_map[variable]})
            step = self.add_universal_instantiation(current_formula, step, new_map[variable])
    ...
    def add_substituted_equality(self, substituted: Union[Formula, str],
        ...
        # Setup
        lhs = equality.arguments[0]
        rhs = equality.arguments[1]
        phi = Formula.FormulaBuilder.equality(parametrized_term.substitute({TermToken.T_UNDERSCORE.value: lhs}),
                                              parametrized_term)

        # Instantiate ME rule with R=phi, c=lhs and d=rhs
        rule = Prover.ME.instantiate({'R': phi, 'c': lhs, 'd': rhs})

        # Build the substitution rules
        step1 = self.add_instantiated_assumption(rule, Prover.ME,  {'R': phi, 'c': lhs, 'd': rhs})
        step2 = self.add_instantiated_assumption(
            Prover.RX.instantiate({'c': phi.arguments[0]}), Prover.RX, {
                'c': phi.arguments[0]
            })
        step3 = self.add_mp(rule.second, line_number, step1)
        return self.add_mp(rule.second.second, step2, step3)
    ...
    def _add_chaining_of_two_equalities(self, line_number1: int,
        ...
        # Kinda proving transitivity rule for equalities - so let's flip-flop this equality
        phi = Formula.FormulaBuilder.equality(Term.parse(TermToken.T_UNDERSCORE.value), equality2.arguments[1])
        formula = Prover.ME.instantiate({
            'R': phi,
            'c': equality2.arguments[0],
            'd': equality1.arguments[0]
        })

        # Resolve the stepps
        step1 = self.add_flipped_equality(
            Formula.FormulaBuilder.equality(equality1.arguments[1], equality1.arguments[0]), line_number1)
        step2 = self.add_instantiated_assumption(formula, Prover.ME, {
            'R': phi,
            'c': equality2.arguments[0],
            'd': equality1.arguments[0]
        })
        step3 = self.add_mp(formula.second, step1, step2)
        return self.add_mp(formula.second.second, line_number2, step3)
    ...
    def add_chained_equality(self, chained: Union[Formula, str],
        ...
        step = self._add_chaining_of_two_equalities(line_numbers[0], line_numbers[1])
        for i in range(2, len(line_numbers)):
            step = self._add_chaining_of_two_equalities(step, line_numbers[i])
        return step

    def get_line(self, index: int) -> Proof.Line:
        """
        Gets an already-inserted line from the prover.
    ...
    def get_lines_count(self) -> int:
        """
        Gets the number of lines this prover currently contains.
